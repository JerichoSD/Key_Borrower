...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $numPart = (int)substr($lastId, 4);
        $numPart += 1;

        $newId = "room" . str_pad($numPart, 1, "0", STR_PAD_LEFT);
    } else {
        $newId = "room1";
    }

    $sql = "INSERT INTO Rooms (Room_id, Name, Status) VALUES ('$newId', '$name', '$status')";
...
<body>
    ...
    <div class="contents">
        <label style="font-size: 100px">Add Room</label>
        <br>
        <form action="add_room.php" method="post">
            <table>
                <th colspan="2"><label>Room Creation Form</label></th>
                <tr>
                    <td><label for="name">Room Name:</label></td>
                    <td><input type="text" id="name" name="name" required></td>
                </tr>
                <tr>
                    <td colspan="2" style="text-align: center;"><input type="submit" name="submit" value="Add Room"></td>
                </tr>
            </table>
        </form>
    </div>
